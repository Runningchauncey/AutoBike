...
def accel_cal():
    ...
    for qq,ax_qq in enumerate(axis_vec): # for every axis with order in axis_vec, order z, y, x
        ax_offsets = [[],[],[]]
        print("-"*50)

        for direc_ii,direc in enumerate(cal_directions): # for every direction with order in cal_directions, order upwards, downwards, horizontally
            ...
            mpu_array = [] # clear array for saving accelerations
            ...
            #example: qq==0, ax_qq=='z', direc_ii==0, direc=="upward", ax_offsets[0] = np.array(mpu_array)[:,cal_indices[0]], cal_indices[0] == 2
            #         ax_offsets[0] = column of az upward, ax_offsets[1] = column of az downward. ax_offsets[2] = column of az perpendicular to gravity

        # Use three calibrations (+1g, -1g, 0g) for linear fit
        popts,_ = curve_fit(accel_fit,                                                          # equation for fitting
                            np.append(np.append(ax_offsets[0],ax_offsets[1]),ax_offsets[2]),    # array concatenated by az_+1g, az_-1g, az_0g
                            np.append(np.append(1.0*np.ones(np.shape(ax_offsets[0])),           # array concatenated by 1, -1, 0
                            -1.0*np.ones(np.shape(ax_offsets[1]))),
                            0.0*np.ones(np.shape(ax_offsets[2]))),
                            maxfev=10000)
        mpu_offsets[cal_indices[qq]] = popts # place slope and intercept in offset array, with order [x, y, z]
    print('Accelerometer Calibrations Complete')
    return mpu_offsets # return popts array with order [x, y, z]
